...
date: 2026-03-26
description: "Как мы перенесли изображения с VPS на S3 + CDN и зачем это нужно для позиций в Google"
readtime: 3
---
...
Сайт на Next.js. Картинок много, сервер один — в одном дата-центре. Пользователь в Новосибирске открывает страницу, и каждая фотография летит из Москвы. PageSpeed ругался на загрузку изображений. Решили это починить.
...
Перенесли все изображения в облачное хранилище Beget S3. Картинки теперь живут отдельно от сервера — VPS разгружен, можно менять хостинг не трогая медиа.

### 2. CDN поверх S3

Включили Beget CDN поверх хранилища. Сеть серверов по стране — картинка отдаётся с ближайшего узла. Пользователь в Новосибирске получает фото не из Москвы, а из Новосибирска.

### 3. Код сайта

Написали функцию `cdnImage()` — она подставляет CDN-домен для всех картинок. Обновили компоненты: блог, страницы услуг, главную. Если CDN-адрес не задан в переменных окружения — сайт работает напрямую с сервера. Переключение — одна строчка в `.env`.
