...
Владелец заметил, что по пятницам заказов вдвое больше — стал запускать рассылки по четвергам.

### Заказы в модалках

15-20 заказов в день. Каждый переход на отдельную страницу крал секунды. Сделали модалки — поменял статус, отредактировал, закрыл — следующий. Turbo Stream, без перезагрузки страницы.
