...
**Фаза 2 — Финансовый контур (2–3 недели).** MCP/скилл `tochka`: выписка, балансы, вебхуки о поступлениях; сверка с реестром договоров (`Documents/contracts/` → структурированный реестр `docs/ai-coo/contracts.yml`: клиент, сумма, периодичность); генератор счетов/актов (шаблоны PDF как в `reports/`); налоговый календарь по профилю **ИП УСН 6%** (`tax-profile.md`): авансы 28.04/28.07/28.10, взносы 57 390 ₽ до 28.12, 1% до 01.07, декларация до 25.04, мониторинг НДС-порога 20 млн ₽. Критерий: «брифинг» показывает вчерашние поступления и должников без ручного ввода.

**Фаза 3 — Автономные циклы (3–4 недели).** launchd-задания: утренний брифинг 09:00, heartbeat 3×/день по борду и Telegram, пятничное ретро; дожимы лидов по расписанию (черновики в очередь на подтверждение). Критерий: неделя работы циклов без вмешательства, ложных алертов <10%.
